...
class DimensionConflictError(_BaseCustomError):
    ...
    _message_template: str = (
        "The shape of this tensor-like is expected as"
        "{req_shape}, but you provide {orig_shape}, the ndim is {orig_ndim},"
        " which is conflict on the number of dimensions."
    )
...
class ShapeValidatedError(_BaseCustomError):
    '''
    If you provide a field as
    `arr: Annotated[TensorLike, tensorshape[3, 4], tensorshape[9, 10]]`.
...
class ShapeUnvalidatedError(_BaseCustomError):
    '''
    If you provide two field: `arr1: Annotated[TensorLike, tensorshape[...]],
    arr2: Annotated[TensorLike, tensorshape['*arrr1']]`.

    You hope `arr2` has same shape with `arr1`,
    but you have a spell error on `arrr1`
...
class ShapeFormatError(_BaseCustomError):
    '''
    If you provide a field:
    `arr: Annotated[TensorLike, tensorshape[3, ..., 4, ...]]`

    or `arr: Annotated[TensorLike,
    tensorshape[3.9, {'a': 13}]]` which is wrong format.

    ShapeFormatError will raised.
    '''
    _error_type: str = 'shape_format'
    _message_template: str = (
        'The tensor shape expression has a wrong format: {reason}'
    )
...
class ShapeConflictError(_BaseCustomError):
    ...
    _message_template: str = (
        'dimension {dim} has length {orig_len}, expected {req_len}'
    )
...
class VarDimentionConflictError(_BaseCustomError):
    '''
    If you provide a field as `arr:
    Annotated[TensorLike, tensorshape[3, ..., 3]],
    ...
    _message_template: str = (
        "The shape of this tensor-like is expected as"
        "{req_shape}, which meas provided ndim should be at least {req_ndim},"
        "but you provide {orig_shape}, the ndim is {orig_ndim},"
        "which is less than the required number."
    )
